...
public class laser extends GameFigureState {
    ...
    public laser(float sx, float sy, float tx, float ty, Color color) {
        super(sx, sy);
        ...
        try {
            laserImage = ImageIO.read(getClass().getResource("laserRed.png"));
        } catch (IOException ex) {
            JOptionPane.showMessageDialog(null, "Error: Cannot open snow.png");
            System.exit(-1);
        }
    }
    ...
    public void render(Graphics2D g) {
        g.setColor(color);
        g.drawImage(laserImage,(int)(super.x - size / 2), 
    ...
    {
        return new Rectangle2D.Float(x - size , y - size, size, size);
    }
